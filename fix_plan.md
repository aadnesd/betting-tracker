...
- [x] **Bankroll available vs open bets breakdown**: The bankroll page currently shows the stored balance per account, but this balance includes stakes that are tied up in open (unsettled) bets. Users need to see the actual available balance vs funds in open bets. For example, if Stake shows balance of 351 USD but only 86 USD is withdrawable because 265 USD is staked on open bets, the UI should display both values.
  
  **DoD:** Bankroll page shows per-account breakdown with: (1) Available Balance = stored balance minus open bet stakes, (2) Open Bets = sum of stakes on unsettled bets for that account. Total Capital card also reflects available vs tied-up funds.
  
  **Why:** Users need accurate visibility into what funds are actually withdrawable vs locked in active positions. Current display overstates available capital.
  
  **Implementation:**
  1. Created `getOpenBetStakesByAccount` query in `lib/db/queries.ts` with `OpenBetStakes` interface (accountId, openBackStake, openLayStake, openLayLiability, totalOpenStake). Query aggregates unsettled bet stakes per account, calculating lay liability as `stake × (odds - 1)` for exchanges.
  2. Updated bankroll page (`app/(chat)/bets/bankroll/page.tsx`) to:
     - Fetch open bet stakes in parallel with other data
     - Show "X in open bets" on Bookmaker Balance card and "Y lay liability" on Exchange Balance card
     - Per-account rows show available balance (total - open stakes) with "of [total]" display when there are open positions
     - Lock icon (🔒) indicates tied-up funds
  
  **Tests:** 4 new tests in `tests/unit/account-queries.test.ts` covering query function signature, `OpenBetStakes` interface structure, totalOpenStake calculation (back stake + lay liability), and stake vs liability distinction. Build passes (14 total tests in file).

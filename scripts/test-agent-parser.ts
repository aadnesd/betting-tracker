...
// Verify env loaded
console.log(`AZURE_DOCUMENT_INTELLIGENCE_ENDPOINT: ${process.env.AZURE_DOCUMENT_INTELLIGENCE_ENDPOINT ? "✓ Set" : "✗ Missing"}`);
console.log(`AI_GATEWAY_API_KEY: ${process.env.AI_GATEWAY_API_KEY ? "✓ Set" : "✗ Missing"}`);

// Now import the parser (after env is loaded)
import { parseSingleBetWithAgent, parseMatchedBetWithAgent, type AgentAccount } from "../lib/bet-parser-agent";
...
async function main() {
  ...
  console.log(`Test accounts: ${testAccounts.map(a => `${a.name} (${a.kind})`).join(", ")}\n`);
  ...
  try {
    ...
    console.log(`OCR: ${result1.ocrDurationMs}ms, LLM: ${result1.llmDurationMs}ms\n`);
    ...
    console.log(`OCR: ${result2.ocrDurationMs}ms, LLM: ${result2.llmDurationMs}ms\n`);

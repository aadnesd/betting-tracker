...
import { and, asc, gte, sql, type SQL } from "drizzle-orm";
...
async function main() {
  ...
  if (linkResult.linkedMatch) {
    console.log(`    Linked: ${linkResult.linkedMatch.homeTeam} vs ${linkResult.linkedMatch.awayTeam}`);
    console.log(`    Date: ${linkResult.linkedMatch.matchDate.toISOString().split("T")[0]}`);
    console.log(`    Competition: ${linkResult.linkedMatch.competition}`);
    if (linkResult.linkedMatch.similarity) {
      console.log(`    Similarity: ${(linkResult.linkedMatch.similarity * 100).toFixed(0)}%`);
    }
  }
  console.log("");
  ...
  if (linkResult.matchId && linkResult.linkedMatch) {
    ...
    const isCorrectMatch = 
      (match.homeTeam.toLowerCase().includes("manchester united") || match.homeTeam.toLowerCase().includes("man utd")) &&
      (match.awayTeam.toLowerCase().includes("manchester city") || match.awayTeam.toLowerCase().includes("man city"));
    ...
    const dateDiff = Math.abs(matchDate.getTime() - expectedDate.getTime()) / (1000 * 60 * 60 * 24);
    
    if (dateDiff < 1) {
      console.log("  ✅ PASS: Match date correct (17 Jan 2026)");
    } else {
      console.log(`  ⚠️  Date differs: got ${matchDate.toISOString().split("T")[0]}`);
    }
  } else {

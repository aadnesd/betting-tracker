...
async function main() {
  ...
  console.log(
    "  - Back bet:",
    backPath,
    `(${(backBuffer.length / 1024).toFixed(1)}KB)`
  );
  console.log(
    "  - Lay bet:",
    layPath,
    `(${(layBuffer.length / 1024).toFixed(1)}KB)`
  );
  ...
  console.log(
    `Back OCR: ${backResult.durationMs}ms, ${backResult.lines.length} lines, ${(backResult.confidence * 100).toFixed(0)}% confidence`
  );
  console.log(
    `Lay OCR: ${layResult.durationMs}ms, ${layResult.lines.length} lines, ${(layResult.confidence * 100).toFixed(0)}% confidence`
  );
  ...
  console.log(
    "Back bet contains team (Real Madrid):",
    backHasTeam ? "✅" : "❌"
  );
  ...
  console.log(
    `Content extraction: ${backHasOdds && backHasTeam && layHasOdds && layHasLiability ? "✅ All checks passed" : "⚠️ Some checks failed"}`
  );

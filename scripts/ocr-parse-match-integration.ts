...
async function runTest(): Promise<void> {
  ...
  console.log(
    `  Football API: ${hasFootball ? "✅ (optional)" : "⚠️ (optional, skipping match link)"}`
  );
  console.log("");
  ...
  console.log(
    `  Back image: bet2.png (${(backBuffer.length / 1024).toFixed(1)}KB)`
  );
  console.log(
    `  Lay image: bet3.png (${(layBuffer.length / 1024).toFixed(1)}KB)`
  );
  console.log("");
  ...
  try {
    ...
    const [backResult, layResult] = await extractTextFromImages([
      backDataUrl,
      layDataUrl,
    ]);
    ...
    console.log(
      `  Back: ${backResult.lines.length} lines, ${(backResult.confidence * 100).toFixed(0)}% confidence`
    );
    console.log(
      `  Lay: ${layResult.lines.length} lines, ${(layResult.confidence * 100).toFixed(0)}% confidence`
    );
    console.log("");
    ...
    results.push({
      ...
      data: {
        backLines: backResult.lines.length,
        layLines: layResult.lines.length,
      },
    });
  } catch (error) {
  ...
  try {
    ...
    console.log(
      `  Duration: ${parseDuration}ms (OCR: ${result.ocrDurationMs}ms, LLM: ${result.llmDurationMs}ms)`
    );
    console.log("");
    ...
    console.log(
      `    Stake: ${result.back.stake} ${result.back.currency || "?"}`
    );
    console.log("");
    ...
    const expectedLayLiability = 11_036.78;

    const oddsMatch =
      Math.abs(result.back.odds - expectedOdds) < 0.01 &&
      Math.abs(result.lay.odds - expectedOdds) < 0.01;
    const stakeMatch = Math.abs(result.back.stake - expectedBackStake) < 1;
    const liabilityMatch =
      result.lay.liability &&
      Math.abs(result.lay.liability - expectedLayLiability) < 1;
    const layCurrencyNok = result.lay.currency === "NOK";

    console.log("  Verification:");
    console.log(
      `    Odds correct (1.46): ${oddsMatch ? "✅" : "❌"} (back: ${result.back.odds}, lay: ${result.lay.odds})`
    );
    console.log(
      `    Back stake correct (~2300.46): ${stakeMatch ? "✅" : "❌"} (${result.back.stake})`
    );
    console.log(
      `    Lay liability correct (~11036.78): ${liabilityMatch ? "✅" : "⚠️"} (${result.lay.liability})`
    );
    console.log(
      `    Lay currency is NOK: ${layCurrencyNok ? "✅" : "❌"} (${result.lay.currency})`
    );
    console.log("");
  ...
  if (hasFootball && parsedData) {
    ...
    try {
      ...
      console.log(
        `  Fetching matches from ${dateFrom.toISOString().split("T")[0]} to ${dateTo.toISOString().split("T")[0]}...`
      );
      ...
      if (!response.ok) {
        throw new Error(
          `Football API returned ${response.status}: ${await response.text()}`
        );
      }
      ...
      const matchingMatches = data.matches.filter(
        (m) =>
      ...
      console.log(
        `  Matches involving "${selection}": ${matchingMatches.length}`
      );
      console.log("");
      ...
      if (matchingMatches.length > 0) {
        ...
        for (const match of matchingMatches.slice(0, 5)) {
          ...
          console.log(
            `    - ${match.homeTeam.name} vs ${match.awayTeam.name} (${match.competition.name}, ${date}, ${match.status})`
          );
        }
      } else {
        console.log(
          `  No upcoming matches found for "${selection}" in major competitions.`
        );
        console.log(
          "  (This match may be in a league not in our sync list, e.g., La Liga 2)"
        );
      }
      console.log("");
      ...
      console.log(
        "    (This is optional - the bet was still parsed correctly)"
      );
      console.log("");
      results.push({
        step: "Match Linking",
...
function printSummary(results: TestResult[], totalStart: number): void {
  ...
  for (const r of results) {
    ...
    console.log(
      `| ${r.step.padEnd(16)} | ${status}     | ${String(r.durationMs).padStart(6)}ms |`
    );
  }

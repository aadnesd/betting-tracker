...
describe("Performance logging instrumentation", () => {
  ...
  describe("screenshots route", () => {
    it("logs performance timing with phase breakdown", async () => {
      ...
      const logCall = consoleSpy.mock.calls.find(
        (call) =>
          ...
          call[0].includes("[screenshots/upload]")
      );
    ...
    it("logs timing phases in correct order", async () => {
      ...
      expect(phases.indexOf("auth=")).toBeLessThan(
        phases.indexOf("parseFormData=")
      );
      expect(phases.indexOf("parseFormData=")).toBeLessThan(
        phases.indexOf("blobUpload=")
      );
      expect(phases.indexOf("blobUpload=")).toBeLessThan(
        phases.indexOf("dbSave=")
      );
    });
  });
  ...
  describe("autoparse route", () => {
    it("logs performance timing with phase breakdown", async () => {
      ...
      (
        dbQueries.getAccountByName as ReturnType<typeof vi.fn>
      ).mockResolvedValue(null);

      (
        parseMatchedBetFromScreenshots as ReturnType<typeof vi.fn>
      ).mockResolvedValue({
        back: {
          type: "back",
          market: "M",
      ...
      const logCall = consoleSpy.mock.calls.find(
        (call) =>
          typeof call[0] === "string" &&
          call[0].includes("[bets/autoparse]") &&
    ...
    it("logs timing phases in correct order", async () => {
      ...
      const phases = phasesMatch?.[1] || "";
      expect(phases.indexOf("auth=")).toBeLessThan(
        phases.indexOf("parsePayload=")
      );
      expect(phases.indexOf("parsePayload=")).toBeLessThan(
        phases.indexOf("fetchScreenshots=")
      );
      expect(phases.indexOf("fetchScreenshots=")).toBeLessThan(
        phases.indexOf("aiParsing=")
      );
      expect(phases.indexOf("aiParsing=")).toBeLessThan(
        phases.indexOf("accountMatching=")
      );
      expect(phases.indexOf("accountMatching=")).toBeLessThan(
        phases.indexOf("updateStatus=")
      );
    });
  });
  ...
  describe("timer utility", () => {
    it("captures total time as sum of phase times", async () => {
      ...
      const sumOfPhases = phaseMatches.reduce(

...
async function main() {
  ...
  try {
    ...
    if (!accountRes.ok()) {
      throw new Error(`Account create failed: ${accountRes.status()} ${await accountRes.text()}`);
    }
    ...
    await page.getByRole('button', { name: /Quick Transaction|Txn/i }).first().click();
    ...
    if (!txRes.ok()) {
      throw new Error(`Transaction create failed: ${txRes.status()} ${await txRes.text()}`);
    }
    ...
    await subTrigger.click();
    await expect(page.getByRole('option', { name: subcategoryName }).first()).toBeVisible({ timeout: 10000 });

    const screenshotPath = `/tmp/bonus-subcategory-verified-${runId}.png`;
    await page.screenshot({ path: screenshotPath, fullPage: false });

    console.log(JSON.stringify({ ok: true, accountName, subcategoryName, screenshotPath }));

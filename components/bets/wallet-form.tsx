...
export function WalletForm({ wallet }: WalletFormProps) {
  ...
  const handleTypeChange = (newType: WalletType) => {
    ...
    const newCurrencies =
      newType === "crypto" ? CRYPTO_CURRENCIES : FIAT_CURRENCIES;
  ...
  return (
    <form className="space-y-6" onSubmit={handleSubmit}>
      ...
      <div className="space-y-2">
        ...
        <Select
          onValueChange={(v) => handleTypeChange(v as WalletType)}
          value={type}
        >
        ...
        <p className="text-muted-foreground text-xs">
          Fiat wallets for Revolut, Skrill, etc. Crypto for Exodus, MetaMask,
          etc.
        </p>
      </div>
      ...
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground text-xs">
          The primary currency for this wallet. Create separate wallets for
